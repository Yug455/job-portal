...
const companyRejister = async(req,res)=>{
 try{ const {companyName,companywebsite,location,description,logo,companyemail} = req.body
 if(!companyName){
...
const companyDeatails = async(req,res)=>{
  try{  const companyDetails=Object.keys(req.body)
    ...
    const companyRejisterDetails=await Company.findOneAndReplace({_id:req.params.id},{
        ... updateData,
        userId:userid,
...
const getCompany = async(req,res)=>{
   try{ const getAllCompany = await Company.find({userId:req.user._id})
...
const getSingleCompany = async(req,res)=>{
 try{
    ...
    const getCompany=await Company.findOne({_id:id,userId:req.user._id})
    if(!getCompany){
        return res.status(404).json({
    ...
    return res.status(200).json({
        "message":"Company Found Succesfully",
        CompanyById:getCompany
    })
 }catch(err){
    res.status(500).json({
        "message":err.message
